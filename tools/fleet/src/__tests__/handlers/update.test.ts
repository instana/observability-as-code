...
describe('handleUpdate', () => {
    ...
    test('successfully sends update request with multiple tags', async () => {
        const postMock = jest.fn().mockResolvedValue({
            data: { requestId: '789', status: 'accepted' }
        });
        mockedAxios.create.mockReturnValue({ post: postMock } as any);

        const argv = { ...baseArgv, tag: ['entity.type=otel-collector', 'region=us-east'] };
        await handleUpdate(argv);

        const [, body] = postMock.mock.calls[0];
        expect(body.tags).toEqual({ 'entity.type': 'otel-collector', 'region': 'us-east' });
    });
    ...
    test('configurationId goes inside args, not at top level', async () => {
        ...
        await handleUpdate(baseArgv);

        const [, body] = postMock.mock.calls[0];
        expect(body.args.configurationId).toBe('ERZU5qxERmKRactcHb3I3w');
        expect(body.configurationId).toBeUndefined();
    });

    test('action is agent.configuration.update, not agent.component.deploy', async () => {
        const postMock = jest.fn().mockResolvedValue({
            data: { requestId: '123', status: 'accepted' }
        });
        mockedAxios.create.mockReturnValue({ post: postMock } as any);
        ...
        const [, body] = postMock.mock.calls[0];
        expect(body.action).toBe('agent.configuration.update');
    });
    ...
    test('throws when tag format is invalid (missing value)', async () => {
        ...
        await expect(handleUpdate(argv)).rejects.toThrow('Invalid tag format');
    });
    ...
    test('sets debug log level when debug flag is true', async () => {
        ...
        await handleUpdate({ ...baseArgv, debug: true });
    ...
    test('logs error and rethrows on HTTP error response', async () => {
        ...
        await expect(handleUpdate(baseArgv)).rejects.toThrow('Bad Request');
    });
    ...
    test('logs error and rethrows on network error', async () => {
        ...
        await expect(handleUpdate(baseArgv)).rejects.toThrow('Network Error');
    });
    ...
    test('logs error and rethrows on non-axios error', async () => {
        ...
        await expect(handleUpdate(baseArgv)).rejects.toThrow('Something unexpected');
    });
});
